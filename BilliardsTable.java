...
public class BilliardsTable extends Engine{
	...
	@Override 
	public void first() {
		//partition size should be at least ball diameter+1 (but don't do this, those poor balls)
		//...you're going to do it, aren't you?
		int partitionSize = 31;
		...
		for(int i = -getWidth()/2; i < getWidth()/2 - partitionSize; i+=partitionSize){
			for(int j = -getHeight()/2; j < getHeight()/2 - partitionSize; j+=partitionSize){
				...
				Vector position = new Vector(
						rand.nextInt(upperX-lowerX) + lowerX,
	...
	private class CueBall implements Comparable<CueBall>{
		...
		/*
		 crucial things to remember about collisions!!
		 1. Object/object collisions-push back your objects before computing the collision to 
		 ensure they don't repeatedly render themselves as intersecting and get stuck together.
		 2. Wall/objects collisions-for the wall collisions, pushing back tends to push the 
		 ball back into neighboring balls, so to avoid them getting stuck inside other balls 
		 and vibrating against the wall, just update the velocity with the corresponding pos/neg 
		 (depending on which wall) ABSOLUTE VALUE of the respective velocity (this is to avoid 
		 the vibrating when it's found to be intersecting with the wall for multiple frames)
		 */
		public void tick(){
			//wall collisions
		...
		//spatial partitioning (quadtrees are more efficient, but it's better than
		//distance comparisons alone)
		...
		private void computeCollision(CueBall ballB){
            ...
            Vector negatedParallelVelA = vel.vectorProjectionOn(distanceBtoA);
            Vector perpendicularVelA = vel.minus(negatedParallelVelA);

            Vector distanceAtoB = ballB.pos.minus(pos);
            Vector negatedParallelVelB = ballB.vel.vectorProjectionOn(distanceAtoB);
